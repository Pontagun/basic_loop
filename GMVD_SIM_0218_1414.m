...
for i=1:1:N-buffSize
    ...
    %% Compute Quaternion
    if(i~=1)
        % UnbiasedXYZ is value of Gyro. Adding 0 to the last elsment to
        % make it pure quarternion.
        w = [UnbiasedXYZ(i,1),UnbiasedXYZ(i,2),UnbiasedXYZ(i,3),0]; % Augment 0 to Angular velocity

        % Quarternion integration, also the first block.
    ...
    %% Ending the first blck, output are (qG, qGA, qGM) as quarternion
    ...
    %% Compute Gravity Vector (y = q' * m * q) -- forword map A_int from inertail frame to body fram by qGA
    a4(i,:) = myQuatProd(myQuatConj(qGA(i,:)),myQuatProd(A_int,qGA(i,:)));
    a3(i,:) = a4(i,1:3); % Convert pure quaternion to 3D Vector

    %% Compute Magnetic North Vector (y= q' * m * q) -- forword map M_int from inertail frame to body fram by qGM
    ...
    %% Cros product of computed gravity vector in boday frame and measured gravity vector.
    ...
    %% Cros product of computed magneto vector in boday frame and measured magneto vector.
    vm2 = m3(i,:);  % computed magneto vector
    vm1 = magnetoAvg(i,:);  % measured magneto vector
    qmv = cross(vm1,vm2);
...
%% Figure 1
figure; plot(magnetInert);title('magnetInert')
minertmag = my3dvnorm(magnetInert);
% minertmagsig = 1./(1+exp(-minertmag()));
% minertmagdsig = minertmagsig .* (1-minertmagsig);
% figure; plot(minertmagsig);
% figure; plot(minertmagdsig);
%% Figure 2
% numvects = varrowtrc(magnetInert);

% vectoaxis = degxyz(magnetInert);
...
figure; plot(NMagnitudeMagInert);title('NMagnitudeMagInert-1')
NMagnitudeMagInert = minertmag / MagMag30;
figure; plot(NMagnitudeMagInert);title('NMagnitudeMagInert-2')
%% Figure 4
angchg = anginertchg(magnetInert);
figure; plot(angchg);title('angchg')
%% Figure 5
